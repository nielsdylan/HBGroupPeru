    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{route('question.store')}}" method="POST" enctype="multipart/form-data" data-form="save-question">
                        <input type="hidden" name="exam_id" value="{{$examen}}">
                        <div class="row">
                            <div class="col-md-8">
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="type_question" name="type_question" class="form-control" data-action="type-question">
                                        ...
                                        @foreach ($types_inputs as $item )
                                            <option value="{{$item->type_question_id}}">{{$item->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
...
<script>
        ...
        '<div class="col-md-10" data-id="'+uniqueId+'">'+
            '<div class="form-check form-check-inline custom-checkbox-textarea">'+
                '<div class="custom-control custom-checkbox custom-checkbox-textarea">'+
                    '<input type="checkbox" class="custom-control-input" id="'+uniqueId+'" name="answer_check[]" value="'+order_answer+'">'+
                    '<textarea class="form-control" name="answer['+order_answer+'][]" ></textarea>'+
    ...
    $(document).on('submit','[data-form="save-question"]',function (e) {
        ...
        var id = $('[name="exam_id"]').val()
        var location_hb = '{{ route('question.exam', ['examen' => 'id'] ) }}';
        location_hb = location_hb.replace('id', id);
